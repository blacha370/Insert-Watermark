...
def find_position(image_path, logo_size, margin, step):
    ...
    for x in range(0, gray.shape[1] - logo_size[1] + 1, step):
        for y in range(0, gray.shape[0] - logo_size[0] + 1, step):
            if np.array_equal(row, gray[y, x:logo_size[1] + x + margin]) and np.array_equal(
                    matrix, gray[y:logo_size[0] + y + margin, x:logo_size[1] + x + margin]):
                return int(x + margin / 2), int(y + margin / 2)


def draw_image(image_path, logo, placement, margin, step, auto=False):
    logo_size = logo.size
    if auto:
...
def draw_images(images: list, logo=None, placement='NW', margin=10, step=20, auto=False):
    ...
    for image_path in images:
        image = draw_image(image_path, logo, placement, margin, step, auto)
        if image:
